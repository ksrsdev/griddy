...
static void TeamSelect_LoadUIStrings(const GameData *data)
{
	...
	//Info Box
	teamSelectData->uiStrings[TEAM_SELECT_UI_INFO_BOX] = "SELECT PLAYER TEAM";
...
static void TeamSelect_UpdateFocusTeam(GameData *data, TeamID id)
{
	...
	if (currFocus == id && id != TEAM_ID_NONE) {
		return;
	}
	...
	if (id == TEAM_ID_RANDOM || id == TEAM_ID_NONE) {
		uiData->hidden = true;
	} else {
		uiData->hidden = false;
	}
...
static void TeamSelect_LoadTeamInfoBox(TeamSelectData *data, TeamID id)
{
	TeamDescription teamDesc = gTeamDescriptions[id];
